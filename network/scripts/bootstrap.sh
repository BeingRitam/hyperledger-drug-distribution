...
createChannel() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    set -x
    peer channel create -o orderer.pharma.net:7050 -c $CHANNEL_NAME -f ./channel-artifacts/channel.tx --outputBlock "./channel-artifacts/$CHANNEL_NAME.block" --tls --cafile $ORDERER_CA >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
joinChannel() {
  ...
  for org in $ORGS; do
    for peer in 0 1; do
      ...
      echo "===================== peer${peer}.${org}.pharma.net  joined channel '$CHANNEL_NAME' ===================== "
      sleep $DELAY
      echo
    done
  done
